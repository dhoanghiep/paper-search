...
class ArxivScraper:
    ...
    def save_papers(self, db: Session, papers_data):
        for paper_data in papers_data:
            ...
            if not existing:
                paper = Paper(arxiv_id=paper_data["id"], title=paper_data["title"], authors=paper_data["authors"], abstract=paper_data["abstract"], published_date=paper_data["published"], pdf_url=paper_data["pdf_url"])
                db.add(paper)
        db.commit()
